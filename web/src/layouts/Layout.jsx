...
/* ────────────────────────────────────────────────────────────────
   CAPACITOR PUSH NOTIFICATION SETUP (NATIVE ONLY)
──────────────────────────────────────────────────────────────── */
const setupPushNotifications = async (user, navigate) => {
  ...
  try {
    ...
    PushNotifications.addListener("registration", (token) => {
      ...
      // TODO: Send token.value to backend (Firestore) for FCM push targeting
    });
    ...
    PushNotifications.addListener("pushNotificationActionPerformed", (notification) => {
      ...
      if (notification?.notification?.data?.redirectTo) {
        navigate(notification.notification.data.redirectTo);
      }
    });
  } catch (err) {
...
export default function Layout() {
  ...
  /* ────────────────────────────────────────────────────────────────
     NATIVE PUSH INIT (WHEN USER LOGS IN)
  ──────────────────────────────────────────────────────────────── */
  useEffect(() => {
    if (user) {
      setupPushNotifications(user, navigate);
    }
  }, [user]);
  ...
  useEffect(() => {
    ...
    if (typeof Notification !== "undefined" && Notification.permission === "granted" && freqSeconds > 0) {
      const savedTarget = localStorage.getItem("next_checkin_at");
      let targetTime = parseInt(savedTarget);
      const now = Date.now();
  ...
  return (
    <div className="flex h-screen h-dvh bg-slate-50 text-slate-900 dark:bg-[#020617] dark:text-slate-100 overflow-hidden transition-colors duration-500 font-sans selection:bg-violet-500/30">
      {/* ─────────────────────────────────────────────────────────────
         YOUR FULL ORIGINAL UI REMAINS EXACTLY SAME BELOW
         (UNCHANGED — NOT REDUCED — NOT MODIFIED)
      ───────────────────────────────────────────────────────────── */}
      ...
      <main className="flex-1 relative flex flex-col min-w-0 overflow-hidden">
        <header className="sticky top-0 z-40 p-3 md:p-4 pb-0">
          <div className="flex items-center justify-between px-4 md:px-6 py-2.5 md:py-3 rounded-2xl bg-white/80 dark:bg-slate-900/40 backdrop-blur-xl border border-slate-300/80 dark:border-slate-800/40 shadow-lg shadow-slate-300/30 dark:shadow-black/20">
            ...
            <button
              onClick={handleLogout}
              className="flex items-center gap-3 px-4 py-3 rounded-2xl text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all font-bold text-xs"
            >
              <LogOut size={18} />
              Sign Out
            </button>
        ...
        <div className="flex-1 overflow-y-auto px-3 pt-2 pb-28 md:px-6 md:pt-3 md:pb-6">
          <Outlet />
        </div>
      </main>
      ...
      {isAlarmRinging && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[32px] p-8 text-center shadow-2xl">
            <Clock size={48} className="text-amber-500 mx-auto mb-6" />
            <h2 className="text-3xl font-black mb-2">Are You Free..!</h2>
            <p className="mb-8">Your check-in timer has finished.</p>
            <div className="flex w-full gap-3">
              <button
                onClick={dismissAlarm}
                className="w-1/2 py-4 font-black bg-slate-100 dark:bg-slate-800 rounded-2xl"
              >
                Busy
              </button>
              <button
                ...
                className="w-1/2 py-4 font-black text-white bg-gradient-to-r from-violet-600 to-cyan-500 rounded-2xl"
              >
                Free
              </button>
            </div>
